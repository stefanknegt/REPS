...
class Policy(nn.Module):
    ...
    def get_action(self, states):
        ...
        '''
        hidden = torch.relu(torch.matmul(self.policy_hidden_weights, states) + self.policy_hidden_bias)
        ...
        #print(torch.min(action), torch.max(action), torch.min(sigma), torch.max(sigma))
    ...
    def back_prop_step(self, begin_states, actions, weights):
        ...
        '''
        N = len(begin_states)
        mu, sigma, _ = self.get_action(begin_states)
